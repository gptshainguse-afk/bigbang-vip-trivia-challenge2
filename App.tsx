        ...
        <main className="flex-1 flex flex-col justify-center relative z-10 overflow-auto">
            ...
            <div className="text-center space-y-12">
              ...
              <div className="flex items-center justify-center gap-20">
                ...
                <div className="text-left space-y-8 max-w-md">
                   ...
                   <button onClick={nextQuestion} disabled={players.length === 0} className="w-full bg-white text-black px-16 py-8 rounded-[2.5rem] font-black text-4xl hover:bg-yellow-400 transition-all shadow-2xl">開始對戰</button>
                </div>
              </div>
            </div>
          )}
          ...
          {gameState === GameState.CHALLENGE_INVITE && (
            <div className="space-y-12 animate-in fade-in max-h-full flex flex-col">
               ...
               <div className="flex justify-center"><button onClick={startBattleAfterInvite} className="bg-white text-black px-32 py-8 rounded-full font-black text-4xl hover:bg-yellow-400 transition-all shadow-2xl active:scale-95">START THE SHOW</button></div>
            </div>
          )}
          ...
          {(gameState === GameState.LEADERBOARD || gameState === GameState.FINISHED) && (
            <div className="space-y-12 animate-in fade-in">
               ...
               <div className="flex justify-center mt-16">{gameState === GameState.LEADERBOARD ? <button onClick={nextQuestion} className="bg-yellow-400 text-black px-32 py-10 rounded-[3.5rem] font-black text-5xl">下一題</button> : <button onClick={fetchNewQuestions} disabled={isSyncing} className="bg-white/10 px-24 py-10 rounded-[3.5rem] font-black text-3xl hover:bg-yellow-400 transition-all hover:text-black">{isSyncing ? '載入中...' : 'NEW BATTLE (10 題)'}</button>}</div>
            </div>
          )}
        </main>
