  ...
  if (role === GameRole.HOST) {
    return (
      <div className="min-h-screen p-10 max-w-7xl mx-auto flex flex-col bg-black">
        ...
        <main className="flex-1 flex flex-col justify-center">
          ...
          {gameState === GameState.CHALLENGE_INVITE && (
            <div className="space-y-12 animate-in fade-in">
               <h2 className="text-[10rem] font-black text-center italic bigbang-yellow uppercase leading-none drop-shadow-2xl">NEW CHALLENGE</h2>
               <div className="max-w-5xl mx-auto space-y-4">
                  <p className="text-white/40 font-bold uppercase tracking-widest text-center mb-8 italic text-2xl">邀請 VIP 進入新一輪 10 題對抗賽</p>
                  {[...players].sort((a,b) => b.score - a.score).map((p, idx) => (
                    <div key={p.id} className="glass-card flex items-center justify-between p-10 rounded-[3rem] border border-white/10">
                      <div className="flex items-center gap-10">
                        <span className="text-5xl font-black text-white/10 w-20">{idx+1}</span>
                        <div>
                          <p className="text-4xl font-black uppercase text-white">{p.name}</p>
                          <p className="text-yellow-400 font-mono text-xl">Score: {p.score}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-5">
                         {p.hasAccepted ? (
                           <span className="bg-green-500 text-black px-10 py-5 rounded-2xl font-black text-2xl animate-pulse">ACCEPTED</span>
                         ) : p.isInvited ? (
                           <span className="bg-white/20 text-white px-10 py-5 rounded-2xl font-black text-2xl italic">INVITED...</span>
                         ) : (
                           <button 
                             onClick={() => sendChallenge(p.id)}
                             className="bg-yellow-400 text-black px-10 py-5 rounded-2xl font-black text-2xl hover:scale-110 active:scale-95 transition-all shadow-xl"
                           >
                             SEND CHALLENGE
                           </button>
                         )}
               ...
               <div className="flex justify-center mt-12">
                 <button 
                   ...
                   className="bg-white text-black px-32 py-10 rounded-[3.5rem] font-black text-5xl hover:bg-yellow-400 transition-all shadow-2xl disabled:opacity-20"
                 >
                   START THE SHOW
                 </button>
               </div>
          ...
          {gameState === GameState.QUESTION && (
            <div className="text-center space-y-12">
              ...
              <div className="flex justify-center gap-10 pt-10">
                ...
                <button onClick={nextQuestion} className="bg-yellow-400 text-black px-24 py-7 rounded-3xl font-black text-4xl">揭曉答案/下一題</button>
              </div>
            </div>
          )}
            ...
            <div className="space-y-12 animate-in fade-in">
               ...
               <div className="flex justify-center mt-16">
                 {gameState === GameState.LEADERBOARD ? (
                   ...
                   <button onClick={fetchNewQuestions} disabled={isSyncing} className="bg-white/10 px-24 py-10 rounded-[3.5rem] font-black text-3xl">
        ...
        <button onClick={() => setShowSettings(true)} className="fixed bottom-12 right-12 p-5 bg-yellow-400 rounded-full shadow-2xl"><svg className="w-12 h-12 text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" /></svg></button>
      </div>
    );
  }
    ...
    if (!currentPlayer) return (
      <div className="min-h-screen flex items-center justify-center p-8 bg-zinc-950">
        <form onSubmit={(e) => { e.preventDefault(); const n = (e.currentTarget.elements.namedItem('playername') as HTMLInputElement).value; if (n) joinGame(n); }} className="w-full max-w-sm space-y-16">
          ...
          <button className="w-full bg-yellow-400 text-black font-black py-10 rounded-[3rem] text-4xl">ENTER</button>
        </form>
      </div>
    );
        ...
        <div className="flex-1 flex flex-col justify-center relative z-10">
          {gameState === GameState.JOINING ? (
            ...
            <div className="space-y-10">
               <div className="text-center bg-white/5 p-12 rounded-[3.5rem] border border-white/10">
