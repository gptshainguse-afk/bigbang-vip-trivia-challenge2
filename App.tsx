...
export default function App() {
  ...
  const handlePlayerAnswer = (playerId: string, answer: string) => {
    console.log(`[Host] Checking answer for ${playerId}: ${answer}`);
    setPlayers(prev => {
      return prev.map(p => {
        if (p.id === playerId) {
          ...
          console.log(`[Host] ${p.name}'s answer: "${answer}" vs Correct: "${correctAns}". Match: ${isCorrect}`);
      ...
      <div className="glass-card w-full max-w-md p-10 rounded-[3rem] border border-yellow-400/30 shadow-[0_0_50px_rgba(255,240,0,0.2)]">
        ...
        <div className="space-y-6">
          ...
          <p className="text-xs text-white/30 leading-relaxed">
            API 金鑰狀態: <span className={customApiKey ? "text-green-400" : "text-yellow-400"}>{customApiKey ? "已使用手動輸入" : "使用環境變數模式"}</span>
          </p>
          ...
          <button 
            onClick={() => broadcast()}
            className="w-full py-3 rounded-xl border border-white/5 text-[10px] text-white/20 font-bold tracking-widest hover:bg-white/5 transition-all"
          >
            FORCE BROADCAST TO ALL PLAYERS
          </button>
        </div>
      </div>
    </div>
  ...
  if (role === GameRole.LOBBY) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-black overflow-hidden">
        <div className="text-center space-y-12 animate-in zoom-in duration-700">
          ...
          <div className="space-y-4">
            ...
            <p className="text-white/20 font-bold uppercase tracking-[0.6em] text-sm animate-pulse">Wait for host to initialize</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen p-10 max-w-7xl mx-auto flex flex-col bg-black">
        <header className="flex justify-between items-end mb-12 border-b-2 border-yellow-400/20 pb-8">
          <div className="flex gap-10 items-end">
            <div>
              <h1 className="text-7xl font-black italic bigbang-yellow tracking-tighter">VIP CENTER</h1>
              <div className="flex items-center gap-4 mt-3">
                ...
                <span className="text-sm text-white/40 font-mono tracking-tighter">ID: {sessionId}</span>
                <span className="text-[10px] px-2 py-0.5 rounded bg-white/5 text-white/40 font-bold border border-white/10">
                  API: {customApiKey ? 'CUSTOM' : 'SYSTEM'}
                </span>
              </div>
            </div>
          </div>
        ...
        <main className="flex-1 flex flex-col justify-center overflow-hidden">
          {gameState === GameState.JOINING && (
            <div className="text-center space-y-12 animate-in zoom-in">
              <h2 className="text-8xl font-black uppercase tracking-tight italic drop-shadow-2xl">SCAN TO JOIN</h2>
              <div className="flex items-center justify-center gap-20">
                <div className="bg-white p-8 rounded-[4rem] shadow-[0_0_150px_rgba(255,240,0,0.3)] border-[12px] border-yellow-400 scale-110">
                  <QRCode value={playerJoinUrl} size={350} />
                </div>
                <div className="text-left space-y-8 max-w-md">
                   <div className="space-y-2">
                     <p className="text-yellow-400 font-black text-2xl uppercase tracking-widest italic">Current VIPs</p>
                     <div className="flex flex-wrap gap-3">
                        {players.map(p => (
                          <div key={p.id} className="bg-white/10 text-white px-5 py-2 rounded-xl font-bold text-lg animate-in fade-in slide-in-from-left-2">{p.name}</div>
                        ))}
                        {players.length === 0 && <p className="text-white/20 italic">Waiting for fans...</p>}
                     </div>
                   </div>
                   <button onClick={nextQuestion} disabled={players.length === 0} className="w-full bg-white text-black px-16 py-8 rounded-[2.5rem] font-black text-4xl hover:bg-yellow-400 transition-all disabled:opacity-20 shadow-2xl active:scale-95">
                    BEGIN SHOW
                   </button>
                </div>
              </div>
          ...
          {gameState === GameState.QUESTION && (
            <div className="text-center space-y-12 animate-in fade-in slide-in-from-bottom-5">
              ...
              <div className="grid grid-cols-2 gap-10 max-w-5xl mx-auto">
                {MEMBERS.map(m => (
                  <div key={m.id} className="glass-card py-24 rounded-[4rem] border border-white/10 text-7xl font-black text-white/90 shadow-2xl relative overflow-hidden group">
                    <div className="absolute inset-0 bg-yellow-400/5 group-hover:bg-yellow-400/10 transition-all"></div>
                    {m.stageName}
                  </div>
                ))}
              </div>
          ...
          {(gameState === GameState.LEADERBOARD || gameState === GameState.FINISHED) && (
            <div className="space-y-12 animate-in fade-in duration-700">
               <h2 className="text-[10rem] font-black text-center italic bigbang-yellow tracking-tighter uppercase leading-none drop-shadow-[0_0_100px_rgba(255,240,0,0.4)]">
                 {gameState === GameState.FINISHED ? 'THE KINGS' : 'RANKINGS'}
               </h2>
               <div className="max-w-5xl mx-auto space-y-6">
                  {[...players].sort((a,b) => b.score - a.score).map((p, idx) => (
                    <div key={p.id} className="glass-card flex items-center justify-between p-12 rounded-[3.5rem] border border-white/10 shadow-2xl animate-in slide-in-from-bottom-10" style={{ animationDelay: `${idx * 100}ms` }}>
                      ...
                      <div className="flex items-center gap-12">
                        <div className="flex gap-3">
                          {idx === 0 && Array(3).fill(0).map((_, i) => <span key={i} className="animate-bounce" style={{ animationDelay: `${i*100}ms` }}>{CROWN_SVG(72, COLORS.GOLD)}</span>)}
                          {idx === 1 && Array(2).fill(0).map((_, i) => <span key={i} className="animate-bounce" style={{ animationDelay: `${i*100}ms` }}>{CROWN_SVG(60, COLORS.SILVER)}</span>)}
                          {idx === 2 && Array(1).fill(0).map((_, i) => <span key={i} className="animate-bounce">{CROWN_SVG(48, COLORS.BRONZE)}</span>)}
                        </div>
                        <span className="text-8xl font-mono text-yellow-400 font-black drop-shadow-[0_0_20px_rgba(255,240,0,0.5)]">{p.score}</span>
                      </div>
                    </div>
                  ))}
               </div>
               <div className="flex justify-center mt-16">
                 {gameState === GameState.LEADERBOARD ? (
                   <button onClick={nextQuestion} className="bg-yellow-400 text-black px-32 py-10 rounded-[3.5rem] font-black text-5xl shadow-[0_20px_50px_rgba(255,240,0,0.4)] hover:scale-105 transition-all">NEXT QUESTION</button>
                 ) : (
                   <button onClick={fetchNewQuestions} disabled={isSyncing} className="bg-white/10 px-24 py-10 rounded-[3.5rem] font-black text-3xl hover:bg-white/20 uppercase tracking-widest border border-white/10">
                     {isSyncing ? 'RE-SYNCING AI...' : 'NEW BATTLE (10 QUESTIONS)'}
                   </button>
                 )}
               </div>
            </div>
        ...
        <button 
          ...
          className="fixed bottom-12 right-12 z-50 p-5 bg-yellow-400 rounded-full shadow-[0_0_50px_rgba(255,240,0,0.5)] hover:rotate-180 transition-all duration-700 active:scale-90"
        >
    ...
    if (!currentPlayer) return (
      <div className="min-h-screen flex items-center justify-center p-8 bg-zinc-950">
        <form onSubmit={(e) => {
          ...
          <input name="playername" required maxLength={12} placeholder="暱稱..." className="w-full bg-white/5 border-2 border-white/10 p-10 rounded-[3rem] text-4xl font-black text-center text-white outline-none focus:border-yellow-400 transition-all shadow-inner uppercase" />
          <button className="w-full bg-yellow-400 text-black font-black py-10 rounded-[3rem] text-4xl shadow-2xl active:scale-95 transition-all uppercase tracking-widest">ENTER ARENA</button>
        </form>
      </div>
    );
      ...
      <div className="min-h-screen p-8 flex flex-col bg-zinc-950 relative overflow-hidden">
        <header className="flex justify-between items-center mb-10 border-b border-white/10 pb-8 relative z-10">
          <div className="flex items-center gap-5">
            ...
            <span className="font-black text-3xl text-white truncate max-w-[200px] uppercase tracking-tighter">{currentPlayer.name}</span>
          </div>
          <div className="text-right relative">
            {(currentPlayer as any).scoreDiff > 0 && (
              <span className="absolute -top-10 right-0 text-4xl font-black text-yellow-400 animate-out fade-out slide-out-to-top-10 duration-1000">
                +100
              </span>
            )}
            <span className="bigbang-yellow font-black text-6xl leading-none">{currentPlayer.score}</span>
            <p className="text-[10px] text-white/30 font-bold uppercase tracking-widest mt-1">Score</p>
          </div>
        </header>
        ...
        <div className="flex-1 flex flex-col justify-center relative z-10">
          {gameState === GameState.JOINING ? (
            <div className="text-center space-y-12">
              ...
              <p className="text-white/40 font-bold text-xl uppercase tracking-widest">Watch the main screen!</p>
            </div>
          ) : gameState === GameState.QUESTION ? (
            <div className="space-y-10 animate-in fade-in slide-in-from-bottom-5 duration-500">
               ...
               {currentPlayer.lastAnswer && (
                 <div className="text-center py-6 space-y-4">
                   <p className="text-yellow-400 font-black text-3xl uppercase italic animate-bounce tracking-widest">SUBMITTED!</p>
                   <p className="text-white/20 text-xs font-bold uppercase tracking-[0.3em]">Checking results on motherboard...</p>
                 </div>
               )}
            </div>
          ) : (
            <div className="text-center space-y-10 animate-in fade-in">
              ...
              <div className="space-y-4">
                 {[...players].sort((a,b) => b.score - a.score).slice(0, 5).map((p, idx) => (
                   <div key={p.id} className={`p-8 rounded-[3rem] flex justify-between items-center transition-all ${p.id === currentPlayer.id ? 'bg-yellow-400/20 border-2 border-yellow-400/50' : 'bg-white/5'}`}>
                      <div className="flex items-center gap-6">
                        ...
                        {p.id === currentPlayer.id && <span className="text-[10px] bg-yellow-400 text-black px-2 py-0.5 rounded font-black">YOU</span>}
                      </div>
                      <span className="font-mono text-yellow-400 font-black text-4xl">{p.score}</span>
                   </div>
                 ))}
              </div>
              <p className="text-white/20 text-sm font-bold uppercase animate-pulse tracking-widest pt-5">Waiting for Host...</p>
            </div>
          )}
        </div>
      </div>
